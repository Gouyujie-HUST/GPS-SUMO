...
<div id="ipcount"> 
	...
	<style>
		/* Global */
		#ViewMain{height:100%;width:100%;}
		#maincontent{ left:20%;width:80%;margin-left:20%; float:left;top:-300px;}
		#fat{left:30%;width:80%; float:left;top:100px;}
		...
		#fat2 { position: relative;left:0%;width:100%;top:-300px;float:left;}
		/* Slider */
		...
		#export {position: relative;left:-50%;top:0%;}
		#pie {position: relative;left:400px;width:700px;top:-350px;}
		...
		#echart1 {float: left; position: relative;overflow-x:auto;width:100%;margin:0 auto;top:-250px;}
		#echart {float: left; position: relative;top:0px;overflow-x:auto;}
		#box {background-color:#FFFFFF; overflow-x:display;left:-50px;top:-130px;}
		
		#export3 {position: relative;left:75%;top:-680px;}
...
if ($_SESSION['len'] > 1000) {
    echo '<b>The result is so big that we will only display 1000 items!</b>';
}
    ...
    <table width='97%' id="table" border='1' cellpadding='0' cellspacing='0' bordercolor='#c9e2e6' bgcolor='#f9f6d3'>
			<thead>
				<tr class="top">
					...
					<th width='12%' bgcolor='#267b9f' scope='col'  style='cursor:pointer'><font color='#ffffff'>Peptide</font></th>
					<th width='8%' bgcolor='#267b9f' scope='col'  style='cursor:pointer'><font color='#ffffff'>Score</font></th>
				  <th width='7%' bgcolor='#267b9f' scope='col'  style='cursor:pointer'><font color='#ffffff'>Cut-off</font></th>
					<th width='13%' bgcolor='#267b9f' scope='col'  style='cursor:pointer'><font color='#ffffff'>Type</font></th>
				...
				<th width='13%' bgcolor='#267b9f' scope='col'  style='cursor:pointer'><font color='#ffffff'>PPI</font></th>
				<th width='12%' bgcolor='#267b9f' scope='col'  style='cursor:pointer'><font color='#ffffff'>Weblogo</font></th>
				</tr>
			</thead>
</div>
</div>
...
$_SESSION['SEQ'][$arrybas[0]]=trim($arrybas[10]);
$_SESSION['siteasa'][$arrybas[0]]=trim($arrybas[9]);
...
for ($k=1; $k<=($eachpage)&&($start+$k)<=$num; $k++) {	
	...
	$arry=explode("\t",$string);
		...
		if ($i==6) {
			if($arry[6]=='NA'){echo "<td align=center>" . "<font face=Courier New >".$arry[6]."</font>". "</td>";}
			else{echo "<td align=center>" . "<font face=Courier New >"."<a href =http://cplm.biocuckoo.cn/View.php?id=".$arry[6]." target='_blank'>".$arry[6]."</a></font>". "</td>";}}
		if($i==7){
			if ($arry[7]=='Exp.'){echo "<td align=center>" . "<font face=Courier New >"."<a href =http://cplm.biocuckoo.cn/View.php?id=".$arry[6]." target='_blank'>".$arry[7]."</a></font>". "</td>";}
			else{echo "<td align=center>" . "<font face=Courier New >".$arry[7]."</font>". "</td>" ;}
		}	
		if ($i!=2&&$i!=6&&$i!=7) { echo "<td align=center>" . "<font face=Courier New >".$arry[$i]."</font>". "</td>" ;}
		}
	if($arry[5]=="Sumoylation"){
		...
		echo "<td><a href='webcomp/weblogo/SIM.png' target='blank'><img src='webcomp/weblogo/SIM.png' width='160'></img></a></td>";
	}
...
//print_r($predsite_json);
//print_r($id_cplm_json);
//print_r($_SEQ_json);
//print_r($_ASA_json);
//print_r($predsite);
...
<button id='changeCPLM' class="btn"  style='font-family:Arial, Helvetica;font-weight: bold; width: 150px; height:30PX;postition:relative;top:10px;left:20px;'>Refresh</button></div>
	...
	<!-- 新加一块3D模型 -->		
	<div id="ViewMain">
				<p><font size='4' face="Arial, Helvetica, sans-serif"><strong>1) Structure&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
				&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;2) Statistics</strong></p>
				<div id='first'>
				<div id='Structure'></div>
				<div id='PTMPDB'></div>
				<div id="pie" style="height:350px;border:3px groove #ebebeb;">
				...
				<div id="pie1" style='position:relatve;top:0%; height:350px;width:350px;' >
					<div id="pie1_can" style='height:350px;width:350px;' ></div>
				</div>	
				<div id="pie2" style='position:relatve;top:-10%; height:350px;width:350px;' >
				...
				<div id='fat2'>
				<p><font size='4' face="Arial, Helvetica, sans-serif"><strong>3) Sumoylation sites/interaction motif in sequence</strong></p>
				...
				<div id='fat'>
				<p id="acce"><font size='4' face="Arial, Helvetica, sans-serif"><strong>4) Accessibility</strong></p>
				<div id='echart1' style='overflow-x:auto;height:400px;margin-left:auto;margin-right:auto;'>
				<div id='echart' style='height:350px;margin-left:auto;margin-right:auto;border:3px groove #ebebeb;' >
				</div>
				</div>
...
<script type='text/javascript'> jQuery.easing={easein:function(x,t,b,c,d){return c*(t/=d)*t+b},easeinout:function(x,t,b,c,d){if(t<d/2)return 2*c*t*t/(d*d)+b;var a=t-d/2;return-2*c*a*a/(d*d)+2*c*a/d+c/2+b},easeout:function(x,t,b,c,d){return-c*t*t/(d*d)+2*c*t/d+b},expoin:function(x,t,b,c,d){var a=1;if(c<0){a*=-1;c*=-1}return a*(Math.exp(Math.log(c)/d*t))+b},expoout:function(x,t,b,c,d){var a=1;if(c<0){a*=-1;c*=-1}return a*(-Math.exp(-Math.log(c)/d*(t-d))+c+1)+b},expoinout:function(x,t,b,c,d){var a=1;if(c<0){a*=-1;c*=-1}if(t<d/2)return a*(Math.exp(Math.log(c/2)/(d/2)*t))+b;return a*(-Math.exp(-2*Math.log(c/2)/d*(t-d))+c+1)+b},bouncein:function(x,t,b,c,d){return c-jQuery.easing['bounceout'](x,d-t,0,c,d)+b},bounceout:function(x,t,b,c,d){if((t/=d)<(1/2.75)){return c*(7.5625*t*t)+b}else if(t<(2/2.75)){return c*(7.5625*(t-=(1.5/2.75))*t+.75)+b}else if(t<(2.5/2.75)){return c*(7.5625*(t-=(2.25/2.75))*t+.9375)+b}else{return c*(7.5625*(t-=(2.625/2.75))*t+.984375)+b}},bounceinout:function(x,t,b,c,d){if(t<d/2)return jQuery.easing['bouncein'](x,t*2,0,c,d)*.5+b;return jQuery.easing['bounceout'](x,t*2-d,0,c,d)*.5+c*.5+b},elasin:function(x,t,b,c,d){var s=1.70158;var p=0;var a=c;if(t==0)return b;if((t/=d)==1)return b+c;if(!p)p=d*.3;if(a<Math.abs(c)){a=c;var s=p/4}else var s=p/(2*Math.PI)*Math.asin(c/a);return-(a*Math.pow(2,10*(t-=1))*Math.sin((t*d-s)*(2*Math.PI)/p))+b},elasout:function(x,t,b,c,d){var s=1.70158;var p=0;var a=c;if(t==0)return b;if((t/=d)==1)return b+c;if(!p)p=d*.3;if(a<Math.abs(c)){a=c;var s=p/4}else var s=p/(2*Math.PI)*Math.asin(c/a);return a*Math.pow(2,-10*t)*Math.sin((t*d-s)*(2*Math.PI)/p)+c+b},elasinout:function(x,t,b,c,d){var s=1.70158;var p=0;var a=c;if(t==0)return b;if((t/=d/2)==2)return b+c;if(!p)p=d*(.3*1.5);if(a<Math.abs(c)){a=c;var s=p/4}else var s=p/(2*Math.PI)*Math.asin(c/a);if(t<1)return-.5*(a*Math.pow(2,10*(t-=1))*Math.sin((t*d-s)*(2*Math.PI)/p))+b;return a*Math.pow(2,-10*(t-=1))*Math.sin((t*d-s)*(2*Math.PI)/p)*.5+c+b},backin:function(x,t,b,c,d){var s=1.70158;return c*(t/=d)*t*((s+1)*t-s)+b},backout:function(x,t,b,c,d){var s=1.70158;return c*((t=t/d-1)*t*((s+1)*t+s)+1)+b},backinout:function(x,t,b,c,d){var s=1.70158;if((t/=d/2)<1)return c/2*(t*t*(((s*=(1.525))+1)*t-s))+b;return c/2*((t-=2)*t*(((s*=(1.525))+1)*t+s)+2)+b},linear:function(x,t,b,c,d){return c*t/d+b}};
	...
	function drawpie(names,dis_site,canvname,tit){
		...
		function drawdisorder($inname,$allk){
			...
			if(dis_site!=''){drawpie(names,dis_site,"pie1_can","Sumoylation Sites");}
			if(dis_sim!=''){drawpie(names,dis_sim,"pie2_can","Sumo-interaction motif");}
			}
	...
	$.ptmSVG=function($width, $data, $select){
		...
		$out=$out + "<defs><style>#load{stroke-dasharray:"+length+"; stroke-dashoffset:"+length+"; animation: strokeanim 3s infinite linear;}#label{display:none}</style></defs>";
	...
	function finalshow($id,$name,$sitelis){
		...
		//}else{
		if($id.search(/^CPLM/)!=-1&&0<$id.substr(4)<209327){ 
			$id=$id.substr(4);
			...
			$.get("File.php",{file:"Basic/Protein-View-"+num+".txt",id:line},function(response){
				...
				//	$("#sideview").show();
					...
					if(all[hash.line]=="-1"){
						...
						$("#Structure").html($mark);
						//alert(all[hash.line]+all[hash.chain]+$sitelis);
		...
		for(var i in sitelist){
			var onesite=sitelist[i]+',K'+sitelist[i]+','+disorderValue[i]+';';
			siteneed=siteneed+onesite;
			}
		disorderValue=disorderValue.slice(0,disorderValue.length-1);
		...
		$datasvg=$name+";;"+$name+";;"+disorderValue.length+";;"+siteneed+";;"+disorderValue+";;"+disorderValue.length;
		
		$(".SVG").attr({id: disorderValue.length});
	...
	var allk=idname[4].replace(/\s+/g,'_');
	var inname=session_id+kk.replace(/\s+/g,'_');
